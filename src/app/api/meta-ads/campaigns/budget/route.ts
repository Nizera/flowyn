...
export async function POST(req: NextRequest) {
  ...
  const localTable = level === 'campaign' ? 'campaigns' : 'ad_sets'
  const localIdField = level === 'campaign' ? 'campaign_id' : 'ad_set_id'
  ...
  await admin.from(localTable).update(updateDb).eq(localIdField, id).eq('ad_account_id', ad_account_id).eq('user_id', user.id)

  return NextResponse.json({ success: true, id, level, daily_budget, lifetime_budget })
}
