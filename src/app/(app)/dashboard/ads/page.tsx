...
export default function AdsPage() {
  ...
  async function handleSync(accountId: string) {
    ...
    try {
      ...
      if (res.status === 429) {
        const resetTime = new Date(data.reset_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
        setSyncError(`Limite de API atingido. Reseta às ${resetTime}`)
      } else if (data.error) {
        setSyncError(data.error)
      } else {
        setCooldowns(prev => ({ ...prev, [accountId]: COOLDOWN_SECONDS }))
        fetchAccounts()
      }
    } catch {
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {accounts.map((account) => {
            ...
            const isDisabled = isSyncing || cooldown > 0
